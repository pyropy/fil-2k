1. 构建镜像
```
git clone https://github.com/filecoin-project/lotus.git && cd lotus && git checkout v1.18.1
wget https://raw.githubusercontent.com/pyropy/fil-2k/master/Dockerfile.lotus-debug
docker build -t lotus:v1.18.1-dev --target lotus -f Dockerfile.lotus-debug .
docker build -t lotus-seed:v1.18.1-dev --target lotus-seed -f Dockerfile.lotus-debug .
docker build -t lotus-miner:v1.18.1-dev --target lotus-miner -f Dockerfile.lotus-debug .
docker build -t lotus-worker:v1.18.1-dev --target lotus-worker -f Dockerfile.lotus-debug .
```
